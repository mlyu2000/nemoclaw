...
- **Agent selector** — `agent: openclaw` (default, NemoClaw/OpenClaw gateway) or
  ...
  pod. Both agents can use the same Telegram bot token and the same LiteLLM
  endpoint.
...
- **Hermes path** (`agent: hermes`): Hermes Agent with OpenAI-compatible API on
  port 8642 + web dashboard on port 18790, served from the same `nemoclaw.<domain>`
  host. The API server key is baked at import time. The chart seeds
...
- **OpenClaw path** (`agent: openclaw`): same as before — static token + UI
  auto-connect via `patch-ui.js` + `auto-token.js`.
- **Hermes path** (`agent: hermes`): uses the NemoClaw Hermes sandbox image
  ...
  dashboard on port 18790 (same VS host).
...
- **Memory**: the OpenClaw gateway's V8 heap is capped by the container memory
  ...
  heap). Without a channel the gateway runs comfortably at 1Gi. Hermes (Python)
  also needs 2Gi for the gateway + dashboard + Telegram long-poll.
...
3. **Fill in the values** in the portal's values form:
   ...
   - `domain.base` (your PCAI base domain), `litellm.apiKey` (the LiteLLM master
     key — fetched from the `litellm-helm-masterkey` secret or pasted),
     `litellm.baseUrl` / `litellm.model`, `persistence.storageClassName`, and
     optional `telegram.*`.
...
1. **App health** — PCAI portal → **Applications → `nemoclaw`** → status
   **ready / ok**. Open the app's **Logs** tab for the agent logs.
2. **Dashboard** — click **"Open"** (or `https://nemoclaw.<domain.base>`).
...
3. **Telegram channel** (if enabled) — the bot sends a **one-off test message**
   on first deploy; after that it replies to DMs / group messages. A pairing code
   means `telegram.allowAll` is `false` — set it to `true` and re-deploy.
